...
> **交接时间**：2026-09-12 ｜ **当前版本 V0.3.8** ｜ harness **209/209**
> **用途**：新对话接手开发。先读本文档，再按需查 SPEC.md（协议）/ LOG-INDEX.md（历史索引）。
> **下一步任务**：账本 + 远方回响（设计素材已备齐，见 §3.1）。
...
| 项 | 值 |
|---|---|
| 源码 | `src/content.js`（单文件 IIFE，约 3500 行，分区 0-10） |
| 产物 | `酒馆助手脚本-副导演.json`（build.mjs 打包，content 内嵌 UserScript） |
| 测试 | `integration-test/harness.html`（209 断言，IAB 运行） |
| 文档 | SPEC.md（协议+版本摘要累积）/ README.md / LOG.md + LOG-INDEX.md |
| Git | master 分支，业务提交与 docs 提交分开，提交后回报短 hash |

**架构一句话**：单一"副导演 API"每 3 楼心跳增量修订世界状态 `$ad_world`（派系暗线三态/事件链/风声/遇敌概率档案）+ 本地骰每楼零 LLM 推进（事件链/风声衰减/区域突发事件）+ 随机遭遇掷骰（用户发送时本地掷骰命中即注入强制开战指令）+ 单世界书词条「副导演」注入正文 + 报纸 UI（📰报纸/⚡事件链/📣风声 三 tab）。

**V0.3.x 核心决策**（不要回退）：敌人安排与开战方式归**正文 AI**（从图鉴自选），副导演只管概率掷骰与世界状态；区域事件**类型本地掷骰 + 内容 LLM 生成**（世界观兼容，类型表用户可编辑）。

## 2. 关键机制地图（content.js 内定位）

| 机制 | 函数/位置 | 要点 |
|---|---|---|
| 世界状态存储 | `CV.world = '$ad_world'` | schema 见 SPEC §4.2；`incident`（区域事件单例）、`floorId`（回滚锚点）挂在顶层 |
...
| 历史 | `pushHistory`/`getHistory` | `$ad_history` 四类（evolve/combat/incident/system）各环形 50 条；🕘 弹窗（设置内入口） |
| 设置 UI | `isMobileDevice()` UA 分流 | PC ⚙ → 960px 八栏目工作台（`.st-*` 类）；移动端 ⚙ → 面板内切 `#panel-view-settings` 4-Tab 零弹窗；`collectFormToSettings(root)` 参数化（只收活跃容器） |
| 报纸 | `renderWire` | activeTab 分流；lead=遇敌几率+区域氛围（`encounterProfile` + 区域事件提醒）；灰卡揭幕体系（`syncRevealState` 接触即揭） |

## 3. 剩余路线图

### 3.1 账本 + 远方回响（⭐ 下一任务，设计素材已备齐）

**世界引擎原版实现**（`World/world-engine-evolution.js:357-447`，已精读）：
- 账本 `ledger`：重大事件（Lv3/4）与风声入账，供远方事件采样；
- 触发：账本条数 ≥ 阈值（`localDistantEventLedgerThreshold` 默认 10）→ 概率掷骰（默认 20%）→ `sampleDistantLedger` 采样（**最近 1/4 必选 + 旧账 Fisher-Yates 洗牌取目标量一半**）→ `buildDistantEventPrompt` 强制推演（50% event / 50% wind 类型）→ 要求返回带 `_distanceGenerated:true` 的 level 2/3 唯一对象 → 校验失败下轮重试（同款 _retry 机制）。

**副导演适配建议**（结合现有架构，供参考、可与用户讨论调整）：
- 账本存储：`$ad_ledger`（新 CV 键）——推演时从 `validateWorld` 阶段把 level≥3 或 stage=爆发 的事件入账（去重，带 round/factions/zone/desc）；
- 触发：挂 dispatchNow（本地骰同款幂等守卫），账本 ≥ 阈值（设置项，默认 10）+ 概率（默认 20%）→ 强制推演 reason='distant-echo'，指令段含**采样出的旧账条目**（要求生成与玩家所在区无关的远方回响：跨区事件萌芽或远方风声）；
- 回执与可见性：走既有 events/winds 通道；报纸远方风声卡自然出现；历史 system 或独立类；
- 防堆叠：远方回响事件 zone 明确不在当前大区（指令段约束）。

### 3.2 之后（优先级递减）
1. **批量回填 backfill**：长聊天冷启动——从某楼起分批补推世界状态（世界引擎 backfillBatchSize 模式）；
2. **时间模式节奏**：推演节律按故事时间（跨日）而非楼数（世界引擎 evolveMode time 模式）；
3. **worldSync 蓝绿灯降噪**：同步词条按关键词扫描注入（世界引擎 worldbookTrigger，`docs/世界书蓝绿灯触发.md` 有完整规范）。
...
> "读 D:\Project\AssistantDirector\HANDOFF.md，然后按 §3.1 的设计开工账本+远方回响"

预计规模：CV 键 + 入账逻辑（validateWorld 后）+ 掷骰触发 + 指令段 + 采样函数 + 历史/UI + harness +10 项左右，一次会话可完成。
