...
> **交接时间**：2026-09-14 ｜ **当前版本 V0.4.5** ｜ harness **259/259**
...
| 测试 | `integration-test/harness.html`（259 断言，IAB 运行） |
...
| 名册除名/拉黑分流 | `removeRosterFaction`/`tombstoneFaction` → `cascadeDeleteFaction` | V0.4.5 双钮分流：✕ 除名=纯移除（出册+级联删暗线，**不进黑名单**——下轮推演模型可重新输出该名并自动登记）；🪦 拉黑=手动黑名单（出册+级联删+禁止复活：推演输入【墓碑（禁止复活）】+ validateWorld 精确同名兜底）；级联删共用（短名容错/事件风声阻力级联/词条重写/报纸去卡）。**已知边界**：黑名单精确匹配拦不住变体名（"伦道夫船运" vs "伦道夫船运（黑法老触点）"）——变体归一化用户明确不做，重复由用户手删 |
...
- [ ] V0.4.5 名册双钮：✕ 除名后条目不进黑名单（下轮推演若模型再输出同名会重新登记，属预期）；🪦 拉黑后推演不再输出该名（输入墓碑名单+输出兜底）
