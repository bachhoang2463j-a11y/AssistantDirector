...
## 2026-09-14 ｜ V0.4.5 墓碑改手动黑名单：除名/拉黑分流（业务提交 `51827db`）

**变更行为**：用户实机截图反馈——AI 每轮推演用变体名重复输出同一批派系（名册「伦道夫船运」在册、「伦道夫船运（黑法老触点）」在墓碑等 4 组），旧语义「除名=自动进墓碑」把用户手动清理的重复条目全部变成了禁止复活名单。改为双钮分流——① **✕ 除名 = 纯移除**（新 `removeRosterFaction`）：从名册删 + 级联删暗线，不进黑名单（下轮推演模型仍可能重新输出该名并自动登记）；② **🪦 拉黑 = 手动黑名单**（原 `tombstoneFaction` 语义不变）：出册 + 级联删 + 禁止复活（推演输入【墓碑（禁止复活）】+ validateWorld 精确同名兜底）；级联删逻辑抽为共用 `cascadeDeleteFaction`（短名容错/事件风声阻力级联/词条即时重写/报纸去卡，原行为逐行保留）。UI：名册行三钮（✏️ 设定/🪦 拉黑/✕ 除名），「墓碑」文案全部改「黑名单」，恢复按钮 title 同步。推演提示词与模型侧【墓碑（禁止复活）】术语不动（既定契约）。变体名归一化**不做**（用户明确：程序限制越多越靠不住，重复自己删）。@version 0.4.5；harness 259/259。

**涉及文件**：`src/content.js`、`integration-test/harness.html`、`酒馆助手脚本-副导演.json`、`LOG.md`、`LOG-INDEX.md`、`HANDOFF.md`

**决策依据**：用户指令「把墓碑改成手动黑名单，不要删掉就自动进墓碑」+ 三问确认（除名=纯移除保留级联删/变体去重不做/入口=行内双钮）。对用户疑问「删掉的重复 AI 还输出吗」的代码结论：原样名双重拦截（提示词铁律 5 + validateWorld 精确匹配 `dead.includes(f.name)`），但变体名可绕过精确匹配——黑名单治不了同义名漂移，此为已知边界（用户接受自删）。

**测试要点**：R2c（V0.4.5 新增）：removeRosterFaction 出册 + 世界状态条目级联删 + 不进 tombstones + 非在册返回 false；R6c：弹窗 [data-del] 点击生效（出册不进黑名单 + 重渲染）；R2/R6b 改测拉黑语义（tombstoneFaction 行为不变）。版本断言 0.4.5。排障：IAB 宿主重置后 harness 页残留旧轮结果（257/v0.4.4），reload 后 evaluate 直点 #run 才跑到新产物——role locator click 被页面上挂着的设置面板遮挡拦截。

**验收依据**：IAB harness 259/259；真机验证留给用户（导入 v0.4.5 后：✕ 除名重复变体名 → 不进黑名单；若下轮模型又输出同名 → 名册自动登记回来属预期；🪦 拉黑 → 推演输入墓碑名单 + 输出兜底丢弃）。
